...
class RealEstateFinancialModel:
    ...
    def render_ai_discovery(self):
        ...
        # Initialize simplified AI discovery
        from utils.simplified_ai_discovery import SimplifiedAIDiscovery
        ai_discovery = SimplifiedAIDiscovery()
        ...
        uploaded_files = st.file_uploader(
            ...
            help="Upload annual reports, quarterly earnings, or analyst reports",
            key="ai_pdf_uploads"
        )
            ...
            if st.button("🚀 Process Documents", type="primary", use_container_width=True):
                ...
                with st.spinner("Reading PDF documents..."):
                    for file in uploaded_files:
                        text = ai_discovery.extract_text_from_pdf(file)
                        if text:
                            documents.append({
                                'name': file.name,
                                'text': text
                            })
                            st.info(f"📄 {file.name}: {len(text):,} characters extracted")
                ...
                if documents:
                    ...
                    with col1:
                        ...
                        with st.spinner("Analyzing business segments..."):
                            segments_df = ai_discovery.analyze_business_segments(documents)
                            if not segments_df.empty:
                                st.session_state.ai_business_segments = segments_df
                        ...
                        with st.spinner("Extracting real estate projects..."):
                            ...
                            if projects:
                                with st.spinner("Merging duplicate projects..."):
                                    merged_projects = ai_discovery.merge_duplicate_projects(projects)
                                st.session_state.ai_real_estate_projects = merged_projects
    ...
    def display_ai_analysis_results(self):
        """Display AI analysis results for business segments and projects"""
        
        # Business Segments Table
        if 'ai_business_segments' in st.session_state and st.session_state.ai_business_segments is not None:
            st.subheader("📊 Business Segments Analysis")
            
            df = st.session_state.ai_business_segments
        ...
        if 'ai_real_estate_projects' in st.session_state and st.session_state.ai_real_estate_projects:
            st.subheader("🏢 Real Estate Projects Summary")
            
            projects = st.session_state.ai_real_estate_projects
            ...
            with col2:
                units_count = df['Total Units'].apply(
                    lambda x: int(x) if str(x).replace(',', '').isdigit() else 0
                ).sum()
            ...
            with col4:
                st.metric("Documents Analyzed", len(st.session_state.get('ai_uploaded_documents', [])))
